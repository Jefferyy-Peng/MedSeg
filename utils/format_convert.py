...
def png2nii(png_path, nii_path):
    ...
    png_files.sort()  # Sort the file names if necessary

    # Read the first image to get the size
    first_img = sitk.ReadImage(os.path.join(png_path, png_files[0]))

    # Create a list to store all the image slices
    image_slices = []

    # Loop through all PNG files and read them as SimpleITK images
    for file_name in png_files:
        # Full path to the current PNG file
        file_path = os.path.join(png_path, file_name)

        # Read the current image slice
        image_slice = sitk.ReadImage(file_path, sitk.sitkUInt8)

        # Ensure that all images have the same size
        assert image_slice.GetSize() == first_img.GetSize(), "Image sizes do not match"

        # Append the current image slice to the list
        image_slices.append(sitk.Cast(image_slice, sitk.sitkFloat32))

    # Stack the 2D slices into a 3D image
    volume = sitk.JoinSeries(image_slices)

    # Write the 3D image to a NIfTI file with .nii.gz extension
    sitk.WriteImage(volume, nii_path, True)
